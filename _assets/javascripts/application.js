...
jQuery(document).ready(function($) { // noconflict wrapper to use shorthand $() for jQuery() inside of this function
    ...
    if(jQuery().cycle) {
        ...
        /*
        $('.cyclable_default .cycle_items').each( function(i) {
            var parent_id = $(this).parent("div.cyclables").attr("id");
            alert(parent_id);
            var expression = $(this).parent("div.cyclables").attr("id",parent_id).children(".cycle_items").children();
            //var expression = $(this).children();
            //alert(expression);
    ...
    $('#sb, .nav_next_previous, #respond').corner("10px");
    $('#ft').corner("top 10px");
